class Board {
  ...
  constructor(grid: string[] = new Array(9).fill(''), size: number = 3) {
    this.grid = grid
    this.size = size
  }
  ...
  makeMove(position: number, symbol: string): Board {
    ...
    return new Board(newGrid, this.size)
  }
  ...
  rows(): string[][] {
    const rows = []
    for (let index = 0; index < this.grid.length; index += this.size) {
      rows.push(this.grid.slice(index, index + this.size))
    }
    return rows
  }
